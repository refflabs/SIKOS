...
function PaymentRow({ booking: b, refetch }) {
  ...
  const handleOnlinePayment = async () => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/bookings/${b.id}/pay`, {
        method: 'POST',
        headers: {
      ...
      if (!res.ok) {
        ...
        const detail = errorData.error ? (typeof errorData.error === 'object' ? JSON.stringify(errorData.error) : errorData.error) : ''
        throw new Error((errorData.message || 'Gagal memulai transaksi') + (detail ? ': ' + detail : ''))
      }
      ...
      toast.error(err.message || 'Terjadi kesalahan saat memproses pembayaran online')
    } finally {
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {(b.status === 'pending' || b.status === 'rejected') && (
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-bold transition-all duration-200 cursor-pointer shadow-sm border border-transparent"
            style={{
              background: 'var(--accent)',
              color: '#ffffff',
            }}
            onMouseEnter={e => { if (!isProcessingPayment) e.currentTarget.style.background = 'var(--accent-hover)' }}
            onMouseLeave={e => { if (!isProcessingPayment) e.currentTarget.style.background = 'var(--accent)' }}
          >
            {isProcessingPayment ? (
              <>
